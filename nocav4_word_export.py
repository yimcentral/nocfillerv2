...
def generate_word():
    """Generate an editable Word version of the same Notice of Completion data."""
    ...
    def add_field_doc(doc, label, value):
        p = doc.add_paragraph()
        label_run = p.add_run(f"{label}: ")
        label_run.bold = True
        p.add_run(safe_text(value, f"[{label} not provided]"))

    def add_selected_doc(doc, label, options_dict):
        selected = [name for name, checked in options_dict.items() if checked]
        if selected:
            add_field_doc(doc, label, ", ".join(selected))

    def add_section(doc, title):
        doc.add_heading(title, level=2)

    def val_or_na_doc(value):
        return safe_text(value, "N/A")

    doc = Document()
    doc_section = doc.sections[0]
    doc_section.top_margin = Inches(0.7)
    doc_section.bottom_margin = Inches(0.7)
    doc_section.left_margin = Inches(0.7)
    ...
    styles["Heading 1"].font.color.rgb = RGBColor(0x00, 0x33, 0x66)
    styles["Heading 2"].font.name = "Arial"
    styles["Heading 2"].font.size = Pt(12)
    styles["Heading 2"].font.bold = True
    styles["Heading 2"].font.color.rgb = RGBColor(0x00, 0x33, 0x66)

    header = doc.add_paragraph()
    header.alignment = WD_ALIGN_PARAGRAPH.CENTER
    header.add_run("California Energy Commission\n").bold = True
    title_run = header.add_run("Notice of Completion & Environmental Document Transmittal")
    title_run.bold = True
    ...
    if os.path.exists(LOGO_PATH):
        try:
            ...
            logo_p.add_run().add_picture(LOGO_PATH, width=Inches(0.85))
        except Exception:
            pass

    add_field_doc(doc, "SCH Number", field("SCH Number", sch_number))

    add_section(doc, "Overview")
    add_field_doc(doc, "Project Title", field("Project Title", project_title))
    add_field_doc(doc, "Lead Agency", "California Energy Commission")
    add_field_doc(doc, "Project Manager", field("Contact Person", contact_name))
    add_field_doc(doc, "Mailing Address", "715 P St, MS 40")
    add_field_doc(doc, "Phone", field("Phone", phone_number))
    add_field_doc(doc, "City", "Sacramento")
    add_field_doc(doc, "ZIP", "95814")
    add_field_doc(doc, "County", "Sacramento")

    add_section(doc, "Project Location")
    add_field_doc(doc, "County", field("County", project_county))
    add_field_doc(doc, "City / Nearest Community", field("City/Nearest Community", project_city))
    add_field_doc(doc, "Cross Streets", field("Cross Streets", cross_streets))
    add_field_doc(doc, "ZIP Code", field("ZIP Code", project_zip))
    lat_long = (f"{latitude.strip()} / {longitude.strip()}" if latitude.strip() and longitude.strip() else "[Coordinates not provided]")
    add_field_doc(doc, "Longitude / Latitude", lat_long)
    add_field_doc(doc, "Total Acres", field("Total Acres", total_acres))
    add_field_doc(doc, "Assessor Parcel No.", val_or_na_doc(assessor_parcel))
    add_field_doc(doc, "Survey Info", f"Section: {val_or_na_doc(section)} | Township: {val_or_na_doc(township)} | Range: {val_or_na_doc(range_)}")
    add_field_doc(doc, "State Highways (within 2 miles)", val_or_na_doc(state_highways))
    add_field_doc(doc, "Airports", val_or_na_doc(airports))
    add_field_doc(doc, "Railways", val_or_na_doc(railways))
    add_field_doc(doc, "Schools", val_or_na_doc(schools))
    ...
    nepa_items = {"NOI": nepa_noi, "EA": nepa_ea, "Draft EIS": nepa_eis, "FONSI": nepa_fonsi}
    ...
    if any(ceqa_items.values()) or (nepa_section and any(nepa_items.values())) or (other_section and any(other_items.values())):
        add_section(doc, "Document Type")
        add_selected_doc(doc, "CEQA", ceqa_items)
        if ceqa_sub_eir and ceqa_prior_sch.strip():
            add_field_doc(doc, "Prior SCH No.", ceqa_prior_sch.strip())
        if nepa_section:
            add_selected_doc(doc, "NEPA", nepa_items)
        if other_section:
            add_selected_doc(doc, "Other", other_items)
    ...
    if any(lat_items.values()):
        add_section(doc, "Local Action Type")
        add_selected_doc(doc, "Local Action Type", lat_items)

    if dev_power or dev_nonpower:
        add_section(doc, "Development Type")
    if dev_power:
        def mw_doc(val):
            return f" ({val.strip()} MW)" if val and val.strip() else ""
        power_lines = []
        if dev_power_wind:
            power_lines.append(f"Wind{mw_doc(dev_power_wind_mw)}")
        if dev_power_solar:
            power_lines.append(f"Solar Photovoltaic{mw_doc(dev_power_solar_mw)}")
        if dev_power_bess:
            power_lines.append(f"Battery Energy Storage System{mw_doc(dev_power_bess_mw)}")
        if dev_power_other and dev_power_other_text.strip():
            power_lines.append(f"Other: {dev_power_other_text.strip()}{mw_doc(dev_power_other_mw)}")
        add_field_doc(doc, "Power", ", ".join(power_lines) if power_lines else "-")
    if dev_nonpower:
        if dev_residential:
            add_field_doc(doc, "Residential", f"Units: {dev_residential_units or '-'} | Acres: {dev_residential_acres or '-'}")
        if dev_office:
            add_field_doc(doc, "Office", f"Sq. Ft.: {dev_office_sqft or '-'} | Acres: {dev_office_acres or '-'} | Employees: {dev_office_emp or '-'}")
        if dev_commercial:
            add_field_doc(doc, "Commercial", f"Sq. Ft.: {dev_commercial_sqft or '-'} | Acres: {dev_commercial_acres or '-'} | Employees: {dev_commercial_emp or '-'}")
        if dev_industrial:
            add_field_doc(doc, "Industrial", f"Sq. Ft.: {dev_industrial_sqft or '-'} | Acres: {dev_industrial_acres or '-'} | Employees: {dev_industrial_emp or '-'}")
        if dev_educational:
            add_field_doc(doc, "Educational", dev_educational_text or "-")
        if dev_recreational:
            add_field_doc(doc, "Recreational", dev_recreational_text or "-")
        if dev_water:
            add_field_doc(doc, "Water Facilities", f"Type: {dev_water_type or '-'} | MGD: {dev_water_mgd or '-'}")
        if dev_transportation:
            add_field_doc(doc, "Transportation", f"Type: {dev_transportation_type or '-'}")
        if dev_mining:
            add_field_doc(doc, "Mining", f"Mineral: {dev_mining_mineral or '-'}")
        if dev_waste:
            add_field_doc(doc, "Waste Treatment", f"Type: {dev_waste_type or '-'} | MGD: {dev_waste_mgd or '-'}")
        if dev_hazardous:
            add_field_doc(doc, "Hazardous Waste", f"Type: {dev_hazardous_type or '-'}")
        if dev_other and dev_other_text.strip():
            add_field_doc(doc, "Other", dev_other_text.strip())
    ...
    if any(issues_checked.values()):
        add_section(doc, "Project Issues Discussed in Document")
        add_selected_doc(doc, "Issues", issues_checked)

    add_section(doc, "Present Land Use / Zoning / GP Designation")
    add_field_doc(doc, "Land Use / Zoning / GP Designation", field("Land Use", land_use))

    add_section(doc, "Project Description")
    add_field_doc(doc, "Description", field("Project Description", project_description))

    add_section(doc, "Reviewing Agencies Checklist")
    agency_map = {
    ...
    if checked_agencies:
        for agency in checked_agencies:
            doc.add_paragraph(agency, style="List Bullet")
    else:
        doc.add_paragraph("[No reviewing agencies selected]")

    add_section(doc, "Local Public Review Period")
    ...
    add_field_doc(doc, "Starting Date", start_str)
    add_field_doc(doc, "Ending Date", end_str)

    add_section(doc, "Lead Agency")
    doc.add_heading("Consulting Firm", level=3)
    add_field_doc(doc, "Firm Name", field("Consulting Firm", la_firm_name))
    add_field_doc(doc, "Address", field("Address", la_firm_address))
    add_field_doc(doc, "City / State / ZIP", field("City/State/ZIP", la_firm_csz))
    add_field_doc(doc, "Contact", field("Contact", la_firm_contact))
    add_field_doc(doc, "Phone", field("Phone", la_firm_phone))

    doc.add_heading("Applicant", level=3)
    add_field_doc(doc, "Applicant Name", field("Applicant", la_app_name))
    add_field_doc(doc, "Address", field("Address", la_app_address))
    add_field_doc(doc, "City / State / ZIP", field("City/State/ZIP", la_app_csz))
    add_field_doc(doc, "Phone", field("Phone", la_app_phone))

    add_section(doc, "Signature of Lead Agency Representative")
...
if st.button("Generate Word", type="secondary", use_container_width=True):
    validation_errors = []
    if ra_caltrans_dist and not ra_caltrans_dist_n.strip():
        validation_errors.append("**Caltrans District #n** — District Number is required.")
    if ra_fish and not ra_fish_n.strip():
    ...
    if validation_errors:
        ...
        try:
            ...
            st.success("Word document generated — checked items appear in the editable document.")
            st.download_button(
                label="Download Notice of Completion Word Document",
                data=word_buffer,
                file_name="notice_of_completion.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                use_container_width=True,
            )
        except ModuleNotFoundError:
            st.error("python-docx is not installed. Add `python-docx` to your requirements file, then redeploy.")
