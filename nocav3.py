...
To run:
    ...
    streamlit run notice_of_completion_app.py
"""
...
with col2:
    lat_site_plan = st.checkbox("Site Plan")
with col3:
    lat_use_permit = st.checkbox("Use Permit")
...
land_use = st.text_area("Land Use Description", placeholder="e.g. Rural/Agricultural, Zoned A-1, GP designation Open Space", height=80)

st.subheader("Project Description")
project_description = st.text_area("Project Description", placeholder="Describe the project in full here...", height=150)
...
def checked_list(options_dict):
    """Return comma-separated string of checked items, or None if none checked."""
...
def generate_pdf():
    ...
    add_field(story, "Contact Person", field("Contact Person", contact_name))
    ...
    # Document Type
    ceqa_checked = {"NOP": ceqa_nop, "Early Cons": ceqa_early_cons, "Neg Dec": ceqa_neg_dec}
    nepa_checked = {"NOI": nepa_noi, "EA": nepa_ea}

    if any(ceqa_checked.values()) or any(nepa_checked.values()):
        story.append(Paragraph("Document Type", heading_style))
        story.append(HRFlowable(width="100%", thickness=0.5, color=colors.lightgrey))
        story.append(Spacer(1, 4))
        add_checked(story, "CEQA", ceqa_checked)
        add_checked(story, "NEPA", nepa_checked)

    # Local Action Type
    lat_checked = {"General Plan Update": lat_gpu, "Site Plan": lat_site_plan, "Use Permit": lat_use_permit}
    if any(lat_checked.values()):
        story.append(Paragraph("Local Action Type", heading_style))
        story.append(HRFlowable(width="100%", thickness=0.5, color=colors.lightgrey))
        story.append(Spacer(1, 4))
        add_checked(story, "Local Action Type", lat_checked)
